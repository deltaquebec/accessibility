...
def check_aria_hidden_focusable(soup, result):
    for el in soup.find_all(attrs={"aria-hidden": "true"}):
        is_focusable = (
            (el.name in {"a", "button", "input", "select", "textarea"} and
             el.get("tabindex", "0") != "-1") or
            bool(el.find(["a", "button", "input", "select", "textarea"]))
        )
        if is_focusable:
...
def audit(url: str) -> AuditResult:
    ...
    check_audit_limitations(soup, result)  # run first!
    check_page_title(soup, result)
    check_language(soup, result)
    check_images(soup, result)
    check_form_labels(soup, result)
    check_buttons(soup, result)
    check_links(soup, result)
    check_headings(soup, result)
    check_iframes(soup, result)
    check_skip_link(soup, result)
    check_tables(soup, result)
    check_aria_roles(soup, result)
    check_autoplay(soup, result)
    check_color_contrast_hints(soup, result)
    check_target_size(soup, result)
    check_viewport(soup, result)
    check_duplicate_ids(soup, result)
    check_dangling_aria_refs(soup, result)
    check_positive_tabindex(soup, result)
    check_aria_hidden_focusable(soup, result)
    check_video_captions(soup, result)
    check_svg_accessibility(soup, result)
    check_meta_refresh(soup, result)
    check_autocomplete(soup, result)
    check_empty_labels(soup, result)
    check_moving_content(soup, result)
    check_image_buttons(soup, result)
    check_audio(soup, result)
    check_image_maps(soup, result)
    check_role_img(soup, result)
    check_object_fallback(soup, result)
    check_fieldsets(soup, result)
    check_details_summary(soup, result)
    check_progress_meter(soup, result)
    check_language_validity(soup, result)
    check_duplicate_adjacent_links(soup, result)
...
COLORS = {"error": "\033[91m", "warning": "\033[93m", "info": "\033[94m", "reset": "\033[0m"}
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        sys.exit("usage: python ally.py <url>")
    url = sys.argv[1]
    ...
    try:
        ...
        hostname = urlparse(result.url).hostname or "audit"
        txt_path = output_dir / f"ally_{hostname}_report.txt"
