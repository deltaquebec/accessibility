...
def extract_structure_tree(pdf_path: str) -> tuple[Optional[StructNode], dict]:
    """extract structure tree; return (root StructNode, info dict)"""
...
def _tree_lines(node: StructNode, prefix: str = "", is_last: bool = True) -> list[str]:
    """recursively build colored tree lines with box-drawing connectors"""
    r = COLORS["reset"]
    dim = COLORS["dim"]
...
def print_tree(node: StructNode):
    ...
    if headings:
        ...
        if h1_count == 0:
            ...
            print(f"\t{warn}{h1_count}x H1 (expected one){r}")
        if skips:
...
def main():
    ...
    def emit(line: str = ""):
        """print to terminal and capture to buffer"""
        print(line)
        buf.write(line + "\n")
