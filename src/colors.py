...
def greedy_select(candidates: np.ndarray, n: int, cvd_types: List[CVD],
    ...
    """
    greedy selection: iteratively pick the color maximizing min distance to selected
        candidates:  array of Lab colors to choose from
        n:           total colors to return (including anchors)
        cvd_types:   CVD types to optimize for
        anchor_labs: pre-selected colors that must be included
...
def parse_color(color_str: str) -> np.ndarray:
    """
    parse a color string and return Lab values; accepts: #RRGGBB, RRGGBB, rgb(R,G,B), R,G,B
    """
...
def generate_palette_image(hex_colors: List[str], rgb_colors: List[Tuple[int, int, int]],
    ...
    """generate PNG image showing color palette"""
    if Image is None:
    ...
    # experimenting with output fonts....
    try:
...
def generate_colorspace_plot(result: dict, output_path: str) -> None:
    """
    visualize selected colors in Lab color space
        left panel: a*b* chromaticity diagram with gamut boundary
        right panel: L* lightness distribution
    """
    # who doesn't have matplotlib???
...
def generate_distinct_colors(n: int,
    ...
    generate n perceptually distinct colors
        ...
        cvd_types:     list of CVD types to optimize for. None = normal vision only
        anchor_colors: list of color strings (#hex or R,G,B) that must be included
...
def parse_color_to_rgb255(color_str: str) -> Tuple[int, int, int]:
    """
    parse any color string to (r, g, b) in 0-255 range;
    reuses parse_color (-> lab) then converts back, so all formats accepted by colors.py work here, too;
    additionally accepts plain css named colors
    """
    import re
...
def generate_contrast_image(palette_hex: List[str], palette_rgb: List[Tuple],
    ...
    """
    generate PNG showing each palette color against reference color, with contrast ratio, wcag pass/fail per row
    """
    if Image is None:
