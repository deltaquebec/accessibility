...
Note: just use a website for this....
...
def gamut_clip_lch(lch: np.ndarray) -> np.ndarray:
    """
    clip LCH color to sRGB gamut by reducing chroma; preserve hue and lightness as much as possible
    """
    L, C, H = lch[0], lch[1], lch[2]
    L = np.clip(L, 0, 100)
...
def parse_color(color_str: str) -> np.ndarray:
    """
    parse a color string and return Lab values
    accepts: #RRGGBB, RRGGBB, rgb(R,G,B), R,G,B
    """
...
def generate_complementary(anchor_lch: np.ndarray, art_mode: bool = False,
    ...
    """generate complementary color (180\degree opposite)"""
    ...
    return ColorHarmony(
        ...
        description="two colors opposite on the color wheel (180\degree apart); high contrast"
    )
...
def generate_analogous(anchor_lch: np.ndarray, angle: float = 30,
                       art_mode: bool = False, anchor_hsl: np.ndarray = None) -> ColorHarmony:
    """generate analogous colors (adjacent hues, \pm30\degree by default)"""
    ...
    return ColorHarmony(
        ...
        colors_lab=colors_lab,
        colors_lch=colors_lch,
        description=f"adjacent hues (\pm{angle}\degree); harmonious, natural"
    )


def generate_triadic(anchor_lch: np.ndarray, art_mode: bool = False,
                     anchor_hsl: np.ndarray = None) -> ColorHarmony:
    """generate triadic colors (120\degree apart)"""
    if art_mode and anchor_hsl is not None:
        H, S, L = anchor_hsl
        colors_hsl = [
            anchor_hsl,
    ...
    return ColorHarmony(
        ...
        colors_lab=colors_lab,
        colors_lch=colors_lch,
        description="three colors evenly spaced (120\degree apart); vibrant, balanced"
    )
...
def generate_tetradic(anchor_lch: np.ndarray, rectangular: bool = False,
    ...
    square: 90\degree apart (H, H+90, H+180, H+270)
    ...
    if rectangular:
        ...
        desc = "four colors evenly spaced (90\degree apart, square); bold, dynamic"
...
def generate_palette_image(harmonies: Dict[str, ColorHarmony], anchor_hex: str,
    ...
    # trying different fonts, can set this later....
    try:
...
def generate_comparison_image(harmonies_lch: Dict[str, ColorHarmony],
    ...
    """generate side-by-side comparison PNG showing LCH and HSL harmonies"""
    if Image is None:
        print("warning: pillow not installed; skipping PNG generation", file=sys.stderr)
        return
...
def generate_colorspace_plot(harmonies: Dict[str, ColorHarmony], anchor_hex: str,
                              output_path: str, mode_label: str = None) -> None:
    """generate visualization of harmonies in Lab color space"""
    ...
    ax2.set_xlabel('a* (green \longleftrightarrow red)', fontsize=11)
    ax2.set_ylabel('b* (blue \longleftrightarrow yellow)', fontsize=11)
...
def generate_comparison_plot(harmonies_lch: Dict[str, ColorHarmony],
                              harmonies_hsl: Dict[str, ColorHarmony],
                              anchor_hex: str, output_path: str) -> None:
    """generate side-by-side colorspace visualization comparing LCH and HSL"""
    if not HAS_MATPLOTLIB:
        print("warning: matplotlib not installed; skipping color space plot", file=sys.stderr)
        return
    ...
    for idx, (harmonies, label) in enumerate([(harmonies_lch, 'lch (perceptual)'),
        ...
        ax.set_xlabel('a* (green \longleftrightarrow red)', fontsize=10)
        ax.set_ylabel('b* (blue \longleftrightarrow yellow)', fontsize=10)
...
def generate_results_txt(harmonies: Dict[str, ColorHarmony], anchor_color: str,
    ...
    """generate text file with harmony results for single mode"""
    lines = []
    lines.append("\n")
    ...
    if mode == 'hsl' and anchor_hsl is not None:
        lines.append(f"hsl values: H={anchor_hsl[0]:.1f}\degree, S={anchor_hsl[1]:.1f}%, L={anchor_hsl[2]:.1f}%")
        lines.append("mode: traditional (HSL)")
    else:
        lines.append(f"lch values: L={anchor_lch[0]:.1f}, C={anchor_lch[1]:.1f}, H={anchor_lch[2]:.1f}\degree")
...
def generate_comparison_txt(harmonies_lch: Dict[str, ColorHarmony],
    ...
    """generate text file comparing LCH and HSL harmonies"""
    lines = []

    lines.append("color harmony palettes comparison")
    lines.append("\n")
    lines.append(f"anchor color: {anchor_color} to {anchor_hex.upper()}")
    lines.append(f"lch: L={anchor_lch[0]:.1f}, C={anchor_lch[1]:.1f}, H={anchor_lch[2]:.1f}\degree")
    lines.append(f"hsl: H={anchor_hsl[0]:.1f}\degree, S={anchor_hsl[1]:.1f}%, L={anchor_hsl[2]:.1f}%")
    lines.append("")
...
def generate_harmonies(anchor_color: str, output_prefix: str = "harmonies",
    ...
    if verbose:
        ...
        print(f"  lch: L={anchor_lch[0]:.1f}, C={anchor_lch[1]:.1f}, H={anchor_lch[2]:.1f}\degree")
        print(f"  hsl: H={anchor_hsl[0]:.1f}\degree, S={anchor_hsl[1]:.1f}%, L={anchor_hsl[2]:.1f}%")
        print(f"  mode: {mode}\n")
...
harmony types generated:
  complementary: 180\degree opposite (2 colors)
  monochromatic: same hue, varying L/C (5 colors)
  analogous: adjacent hues \pm30\degree (5 colors)
  triadic: 120\degree apart (3 colors)
  tetradic: 90\degree apart, square (4 colors)
